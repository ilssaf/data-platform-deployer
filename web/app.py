...
st.markdown("""
Это веб-интерфейс для вашего CLI-инструмента `dpd`.
...
project_description = st.text_area("Описание (project.description)", height=68, value="Descritpion of the project")
...
config = {
    ...
    "storage": {
        "clickhouse": {"name": ch_name}
    },
    "bi": {
        "superset": {"name": superset_name}
    }
}
...
if st.button("🚀 Сгенерировать и скачать проект"):
    try:
        with st.spinner("Генерация проекта…"):
            ...
            zip_path = shutil.make_archive(base_name=archive_base,
                                           format="zip",
